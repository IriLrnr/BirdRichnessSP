...
cdata <- cdata %>%
  ...
  mutate(re = richness/effort) %>% 
  mutate(relog = richness/log(effort))
```
...
```{r, echo=FALSE, warning=FALSE}
# Variáveis selecionadas pelo Lasso
sp_sh_gam <- c("s(SEVIn,SVI)")

# Criar fórmula para o modelo GAM
gam_formula <- as.formula(paste("relog ~", paste(sp_sh_gam, collapse = " + ")))

# Ajustar o modelo GAM usando as variáveis selecionadas pelo Lasso
gam_p <- gam(gam_formula, data = cdata, family = poisson(link = "log"))

# Resumo do modelo
summary(gam_p)

# Plotar
plot(gam_p,all.terms = TRUE)
vis.gam(gam_p, theta = 150, n.grid = 50, lwd = 0.4)

```
```{r, echo=FALSE, warning=FALSE}
sim_data <- expand.grid(SEVIn = seq(-1, 1, length.out = 100),
                        SVI = seq(0, 1, length.out = 200))

# Use predict() to generate predictions
sim_data <- sim_data %>% 
  mutate(selog = predict(gam_p, newdata = sim_data))

# Plot the results
ggplot(data = sim_data) +
  geom_point(aes(x = SVI, y = SEVIn, color = selog)) +
