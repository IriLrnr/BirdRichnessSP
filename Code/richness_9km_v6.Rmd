...
shp <- st_read("C:/Users/Bianca/Documents/USP/Projeto/eBird/Data/3km/richness_effort_v16_total_3km_urb_f5km_30min_10spp.shp")
cdata <- shp
...
cdata <- cdata %>%
  ...
  mutate(re = richness/log(effort)) %>% 
  mutate(re2 = richness/effort) 
...
```
## Association between richness/effort and variables
```{r, echo = FALSE, message=FALSE}
# Create a new table with NA values replaced by 0 in the specified predictor columns
cdata1 <- cdata %>%
  mutate(across(all_of(c(structure, dynamics, composition, configuration)), ~ replace_na(., 0)))
...
# Reshape the data to long format with only 'value' and 'variable'
cdata_long <- cdata1 %>%
  select(re2, all_of(c(structure, dynamics, composition, configuration))) %>%
  pivot_longer(
    cols = c(structure, dynamics, composition, configuration),
    names_to = "variable",
    values_to = "value"
...
# Create faceted plot for the 'structure' group
plot_structure <- ggplot(cdata_long %>% filter(variable %in% structure), aes(x = value, y = re2)) + 
  geom_point(alpha = 0.7, color = "black") + 
  geom_smooth(method = "loess", color = "blue", fill = "blue", alpha = 0.1) +
  facet_wrap(~ variable, scales = "free_x", ncol = 3) + # Adjust the number of columns as needed
  labs(x = "Value", y = "re") +
  ...
  ggtitle("re vs. Structure Predictors")

# Create faceted plot for the 'dynamics' group
plot_dynamics <- ggplot(cdata_long %>% filter(variable %in% dynamics), aes(x = value, y = re2)) + 
  geom_point(alpha = 0.7, color = "black") + 
  geom_smooth(method = "loess", color = "blue", fill = "blue", alpha = 0.1) +
  facet_wrap(~ variable, scales = "free_x", ncol = 4) + # Adjust the number of columns as needed
  labs(x = "Value", y = "re") +
  ...
  ggtitle("re vs. Dynamics Predictors")

# Create faceted plot for the 'composition' group
plot_composition <- ggplot(cdata_long %>% filter(variable %in% composition), aes(x = value, y = re2)) + 
  geom_point(alpha = 0.7, color = "black") + 
  geom_smooth(method = "loess", color = "blue", fill = "blue", alpha = 0.1) +
  facet_wrap(~ variable, scales = "free_x", ncol = 2) + # Adjust the number of columns as needed
  labs(x = "Value", y = "re") +
  ...
  ggtitle("re vs. Composition Predictors")

# Create faceted plot for the 'configuration' group with limited y-axis
plot_configuration <- ggplot(cdata_long %>% filter(variable %in% configuration), aes(x = value, y = re2)) + 
  geom_point(alpha = 0.7, color = "black") + 
  geom_smooth(method = "loess", color = "blue", fill = "blue", alpha = 0.1) +
  facet_wrap(~ variable, scales = "free_x", ncol = 4) + # Adjust the number of columns as needed
  labs(x = "Value", y = "re") +
  ...
  ggtitle("re vs. Configuration Predictors") +
  coord_cartesian(ylim = c(0, max(cdata$re, na.rm = TRUE))) # Limiting the y-axis
...
```
We see that there is no spatial correlation
...
```{r, echo=FALSE, warning=FALSE}
selected <- glm(formula = richness ~ SUVI + SVI + SGVI + shdi + effort + 
    pop + Wdistance + Gdistance + Bvol + Bheight_me + Vvol + 
    prop_veg + pland_tot + lpi_tot + ed_tot + contig_mn_ + nlsi_tot + 
    np_tot + area_mn_tot, family = poisson(link = "log"), data = cdata)
summary(selected)
...
deviance_model <- sh_sp_model$deviance

# Get the deviance of the null model (a model with no predictors)
deviance_null <- sh_sp_model$null.deviance

# Calculate the deviance R²
r_squared <- 1 - (deviance_model / deviance_null)
...
lasso_vars <- c("SVI", "shdi", "effort", "Vheight_me", "nlsi_tot")
